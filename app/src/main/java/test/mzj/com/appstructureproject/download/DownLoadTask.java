...
public class DownLoadTask extends Thread {
    ...
    private DownDao helper;//数据库帮助类
    private int finished = 0;//当前已下载完成的进度
    private OnProgressListener listener;//进度回调监听

    public DownLoadTask(FileInfo info, DownDao helper, OnProgressListener listener) {
        this.info = info;
        this.helper = helper;
            ...
            if (connection.getResponseCode() == 206) {//文件部分下载，返回码为206
                ...
                while ((len = is.read(buffer)) != -1) {
                    ...
                    if (info.isStop()) {
                        ...
                        //保存此次下载的进度
                        helper.update(info.getLength(),info.getFinished(),info.getUrl());
                        return;
                    }
                }
                //下载完成
